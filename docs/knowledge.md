...
Yes, we should write down features and decisions as people build them. No, that knowledge should not live in this coboose.

A central wiki here would be a third source of truth (code, Jira, coboose docs). It will rot, and Copilot will start trusting a stale copy over the repo that actually changed.
...
1. Vague prompt → `coboose context` → read Graphify reports **and** `knowledge.files`.
