...
The goat talks to Figma with a **personal access token** in the `X-Figma-Token` header. Copilot never sees the token.
...
4. Name it something obvious, for example `goat`.
5. Set an expiry.
...
If your company uses SSO, you can still create a token for your own Figma account. This goat is meant for personal tokens, not a shared bot.
...
```bash
uv run goat figma login --from-env
```
...
| --- | --- | --- |
| macOS | Keychain | service `goat`, account `figma-access-token` |
| Windows | Credential Manager | generic credential `goat` / `figma-access-token` |
| Linux desktop | Secret Service (GNOME Keyring / KWallet) | same service and account |
| Headless / CI | `.env` fallback | `FIGMA_ACCESS_TOKEN=...` |

This name is a row in `catalog/env.yaml`. `uv run goat env list` shows whether it is present without printing the value.

## 3. Confirm it works

```bash
uv run goat figma whoami
uv run goat doctor --ping-figma
```
...
2. Create a new one.
3. Run `uv run goat figma login` again (or `figma login --from-env` if you temporarily put the new value in `.env`).

```bash
uv run goat figma logout
```
