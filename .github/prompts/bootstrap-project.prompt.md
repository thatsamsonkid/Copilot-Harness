...
1. From the harness repo (do not `cd` into a sibling first), run `#tool:runCommands` with `uv run harness templates --format json`. If cwd is already a product clone, use `uv run --project "$HARNESS_ROOT" harness templates --format json`.
