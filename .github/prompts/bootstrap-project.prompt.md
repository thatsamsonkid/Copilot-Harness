...
description: List coboose templates and bootstrap a new project under parent_dir from one of them
...
1. From the coboose repo (do not `cd` into a sibling first), run `#tool:runCommands` with `uv run coboose templates --format json`. If cwd is already a product clone, use `uv run --project "$COBOOSE_ROOT" coboose templates --format json`.
...
4. Run `#tool:runCommands` with `uv run coboose bootstrap --template ${input:template:Template name from templates.yml} --name ${input:name:New folder name or group/name} --format json`. Add `--group` when they want organized folders.
5. Do not nest the new project inside this coboose. Do not `git clone` by hand when `coboose bootstrap` can do it.
