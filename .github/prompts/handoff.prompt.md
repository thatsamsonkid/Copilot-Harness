...
3. Run `uv run coboose handoff write --issue ${input:issue:} --note "<paragraph>" --format json` (omit `--issue` if there is no key).
4. Tell them the `relative` path and that the file is gitignored.

If they are **resuming** or did not say write:

1. Run `uv run coboose handoff latest --format json`. If none exists, say so and run `status` instead.
2. Refresh `uv run coboose status --format json`.
3. If there is a Jira key, run `uv run coboose prepare <KEY> --format json` and list unchecked `done_when` items.
