...
Copilot collects the remaining params in chat, then runs the harness CLI. Do **not** run interactive `harness workspace create` (no TTY). Do **not** hand-edit `catalog/stack.yaml` or `workspaces/*.code-workspace`.
...
6. Restate the plan (id, name, folders, harness yes/no) and wait for a short confirm unless they already said "just create it".
...
```
   ...
   Add `--tag`, `--no-include-harness`, `--fallback`, or `--force` only when the user chose those.
