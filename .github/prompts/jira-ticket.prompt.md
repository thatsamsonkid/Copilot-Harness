...
1. From the harness repo (do not `cd` into a sibling first), run `#tool:runCommands` with cwd = the harness folder and `uv run harness prepare ${input:issue} --format json`. If cwd is already a product clone, use `uv run --project "$HARNESS_ROOT" harness prepare ${input:issue} --format json` instead — bare `uv run harness` cannot spawn there.
