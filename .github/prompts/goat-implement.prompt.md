---
name: goat-implement
description: Start implementing an agreed plans/ file — fan out Code Writers per Parallel waves row, then Verifier (survives chat compaction)
...
4. Walk **Parallel waves** in order. For every product-file write, invoke **Code Writer** (`#tool:agent`) — one call per independent step in the wave, issued together in the same turn — with that step's spec, target paths, and reference paths. Do not send two writers the same file. Do not `#tool:edit` product files yourself. Do not spawn Implementer. You still write goat catalog, feature notes, and ADRs yourself. If **Parallel waves** is missing, treat each step as its own wave.
5. After every writer in a wave returns, invoke **Verifier** (`#tool:agent`) with that wave's Verify commands, cwds, and expected results (fan out Verifiers when repos/cwds differ). After all waves, invoke Verifier with the plan **Verification** section and each repo's `tooling.suggested_verify`. If Verifier reports fail, stop and report — do not start the next wave.
