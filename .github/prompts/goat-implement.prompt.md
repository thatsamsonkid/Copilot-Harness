...
4. For every product-file write, invoke **Code Writer** (`#tool:agent`) with the spec, target paths, and reference paths. Do not `#tool:edit` product files yourself — the `implement-gate` hook will deny those parent edits. Do not spawn Implementer. You still write goat catalog, feature notes, and ADRs yourself.
