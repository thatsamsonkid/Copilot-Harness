...
4. Start backends first, one process at a time, **one VS Code terminal per app**. If `run_via` is `harness`, run `copilot_command` (`uv run --project "$HARNESS_ROOT" harness start run --repo <name>`) from the harness cwd — the CLI starts the process in the repo folder and applies launch env to that child. To inspect keys/collisions without starting, run `harness start env --repo <name> --format json`. To apply env in a terminal without starting the app, run `harness start env --repo <name> --shell`. If `run_via` is `vscode`, use **Run Without Debugging** on the named launch configuration (not Debug unless asked). If `run_via` is `terminal`, run `command` as usual. If a terminal already exists for that repo, reuse it and read its log. Never start a second long-running app in a busy terminal.
