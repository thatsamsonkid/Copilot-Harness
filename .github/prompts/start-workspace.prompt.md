...
1. Run `#tool:runCommands` with cwd = the coboose workspace folder (first root, named `coboose`) and `uv run coboose start --format json` (add `--workspace <id>` or `--repo <names>` when the user named them). The command follows `COBOOSE_WORKSPACE` when a feature `.code-workspace` is open. If `workspace_scope.detected` is false, pass `--workspace <id>` or ask — do not plan every clone on disk. Do not `cd` into a product repo first. If cwd is already a sibling, use `uv run --project "$COBOOSE_ROOT" coboose start --format json` or `./scripts/coboose.sh start --format json` (Windows: `.\scripts\coboose.ps1`). Bare `uv run coboose` cannot spawn from a sibling.
2. If `plan_source` is `saved`, follow that sequence. If it is `discovered` and the order looks right, save it once with `uv run coboose start --save --format json` so later starts skip rediscovery (`workspaces/<id>.start.yml`).
