...
- Vague, broad, or no-ticket prompts in a large workspace: load `.github/skills/workspace-context/SKILL.md` and run `uv run goat context --format json`. Stay inside `workspace.repos`. If `workspace_scope.detected` is false, run `goat workspace current` and ask which feature workspace to open — do not treat every clone under `parent_dir` as in scope. Read each listed repo's Graphify `GRAPH_REPORT.md` before grepping. When planning a feature (Jira Planner or a ticket key), automatically invoke **Bulk Reader** for **survey** questions only — do not wait for the user to pick an orchestrator. Do not delegate debugging, architectural decisions, or safety-critical analysis; targeted-`Read` those sections yourself (`limit`/`offset`). Never edit from Bulk Reader bullets — re-read the slice first. After `/goat-plan`, never spawn Implementer. If you wrote the plan in this chat, you become Implementer — invoke Code Writer for product files; you still run `goat branch`, verify, and feature notes. If this is a new chat handed only a `plans/` file, you are the executor: write product files yourself; do not spawn Code Writer. Subagents must not spawn other subagents. Do not treat Code Writer as a second Implementer. If a whole-file `Read` or `cat` is denied as too large and the task is a survey, load `.github/skills/bulk-reader/SKILL.md` or `/bulk-reader`. If the task is debug / architecture / safety / an edit, retry with `limit`/`offset` instead.
...
- Agreed-plan writes after `/goat-plan`: never spawn Implementer. Same chat that wrote the plan: you become Implementer and invoke **Code Writer** for product files (you still branch, verify, and write feature notes). New chat with only a `plans/` file: you are the executor — write product files yourself; do not spawn Code Writer. VS Code Agents dropdown: user picks **Implementer** as primary (same as the same-chat path).
