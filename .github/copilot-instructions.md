...
- VS Code Agents cannot see skills in multi-root child folders: load `.github/skills/skills-install/SKILL.md` and run `uv run coboose skills list --brief --format json`. From chat, lift with `coboose skills lift --only` or `--all-skills`. In a terminal, `coboose skills lift` prompts (numbers, names, `all`). Or `coboose skills pull <git-url>` then `--only`. `init` / `prepare` / `workspace generate` already copy coboose + in-scope sibling skills into this repo's `.github/skills`. Do not commit those copies.
