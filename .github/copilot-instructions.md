...
5. Write a plan covering impacted repos, likely files, risks, and tests. Do not implement until the user asks. If the plan should be saved for later or handed to another model, load `.github/skills/planning/SKILL.md` and write it to `plans/` (gitignored) from `templates/plan.md` — detailed enough for a zero-context executor. When they ask to implement (especially after chat compaction), load `.github/skills/implementing/SKILL.md` or `/goat-implement` — become Implementer, invoke Code Writer and Verifier; do not spawn Implementer.
...
- Feature planning: **Jira Planner** or `/jira-ticket`. It automatically delegates product reads to **Bulk Reader**.
- Agreed-plan writes after `/goat-plan`: never spawn Implementer. Start with `/goat-implement` (survives compaction): you become Implementer, invoke **Code Writer** for product files and **Verifier** for verify checks (you still branch and write feature notes). New chat with only a `plans/` file and no `/goat-implement`: you are the executor — write product files yourself; do not spawn Code Writer or Verifier. VS Code Agents dropdown: user picks **Implementer** as primary (same as `/goat-implement`).
- Large-file read blocked: `/bulk-reader` or the bulk-reader skill. Hooks live in `.github/hooks`.
- `/goat-implement` write gate: `implement-gate` denies parent `#tool:edit` on sibling product files. Invoke **Code Writer**. Feature notes, ADRs, and goat catalog still pass. `GOAT_IMPLEMENT_EDIT_GATE=off` disables the deny.
