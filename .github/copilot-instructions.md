...
- Vague, broad, or no-ticket prompts in a large workspace: load `.github/skills/workspace-context/SKILL.md` and run `uv run goat context --format json`. Stay inside `workspace.repos`. If `workspace_scope.detected` is false, run `goat workspace current` and ask which feature workspace to open — do not treat every clone under `parent_dir` as in scope. Read each listed repo's Graphify `GRAPH_REPORT.md` before grepping. For many-file questions, pick the **Orchestrator** agent (or `/orchestrate`) so **Bulk Reader** fills context without dumping source into this chat.
...
- Many-file context without a source dump: **Orchestrator** agent or `/orchestrate`. File reads go to the hidden **Bulk Reader** subagent.
