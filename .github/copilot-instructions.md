...
- Agreed-plan writes after `/goat-plan`: the current chat agent is the executor (edit product files, `goat branch`, verify, feature notes). Do not spawn Implementer or Code Writer. VS Code Agents dropdown only: the user picks **Implementer** as the primary chat; only that primary may invoke **Code Writer** for product files.
