...
- Agreed-plan writes after `/goat-plan`: never spawn Implementer. Start with `/goat-implement` (survives compaction): you become Implementer, fan out **Code Writer** per independent step in a **Parallel waves** row, then **Verifier** for that wave (you still branch and write feature notes). New chat with only a `plans/` file and no `/goat-implement`: you are the executor — write product files yourself; do not spawn Code Writer or Verifier. VS Code Agents dropdown: user picks **Implementer** as primary (same as `/goat-implement`).
