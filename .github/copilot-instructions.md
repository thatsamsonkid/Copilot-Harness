...
- Vague, broad, or no-ticket prompts in a large workspace: load `.github/skills/workspace-context/SKILL.md` and run `uv run goat context --format json`. Stay inside `workspace.repos`. If `workspace_scope.detected` is false, run `goat workspace current` and ask which feature workspace to open — do not treat every clone under `parent_dir` as in scope. Read each listed repo's Graphify `GRAPH_REPORT.md` before grepping. When planning a feature (Jira Planner or a ticket key), automatically fan out **Bulk Reader** for **survey** questions only (one call per independent file group) — do not wait for the user to pick an orchestrator. Do not delegate debugging, architectural decisions, or safety-critical analysis; targeted-`Read` those sections yourself (`limit`/`offset`). Never edit from Bulk Reader bullets — re-read the slice first. After `/goat-plan`, never spawn Implementer. Start implementation with `/goat-implement` (survives chat compaction): you become Implementer — fan out Code Writers per Parallel waves row, then Verifier (one bounded repair per step, then you investigate); you still run `goat branch` and feature notes. If this is a new chat handed only a `plans/` file and they did not run `/goat-implement`, you are the executor: write product files yourself; do not spawn Code Writer or Verifier. Subagents must not spawn other subagents. Do not treat Code Writer or Verifier as a second Implementer. If a whole-file `Read` or `cat` is denied as too large and the task is a survey, load `.github/skills/bulk-reader/SKILL.md` or `/bulk-reader`. If the task is debug / architecture / safety / an edit, retry with `limit`/`offset` instead.
...
5. Write a plan covering impacted repos, likely files, risks, and tests. Do not implement until the user asks. If the plan should be saved for later or handed to another model, load `.github/skills/planning/SKILL.md` and write it to `plans/` (gitignored) from `templates/plan.md` — detailed enough for a zero-context executor, with **Survey groups** for Bulk Reader fan-out and independent steps grouped into **Parallel waves**. When they ask to implement (especially after chat compaction), load `.github/skills/implementing/SKILL.md` or `/goat-implement` — become Implementer, fan out Code Writers per wave, then Verifier (one bounded repair per step, then you investigate); do not spawn Implementer.
...
- Workspaces / Jira routing: `catalog/stack.yaml` (shipped) plus optional `catalog/stack.local.yaml` (personal, gitignored) — reference repos by name or tag.
...
- Feature workspaces: `catalog/stack.yaml` is the committed team catalog. `goat workspace create` writes `catalog/stack.local.yaml` (gitignored) unless `--shared`. `workspaces/*.code-workspace` files are generated locally (gitignored); first folder is this Goat repo.
...
- Feature planning: **Jira Planner** or `/jira-ticket`. It automatically fans out **Bulk Reader** for independent survey groups.
- Agreed-plan writes after `/goat-plan`: never spawn Implementer. Start with `/goat-implement` (survives compaction): you become Implementer, fan out **Code Writer** per independent step in a **Parallel waves** row, then **Verifier** for that wave (one bounded repair per step, then you investigate; you still branch and write feature notes). New chat with only a `plans/` file and no `/goat-implement`: you are the executor — write product files yourself; do not spawn Code Writer or Verifier. VS Code Agents dropdown: user picks **Implementer** as primary (same as `/goat-implement`).
- Large-file read blocked: `/bulk-reader` or the bulk-reader skill. Hooks live in `.github/hooks`.
