...
- After catalog edits, run `goat workspace generate`. To add a workspace, prefer `/new-workspace` or the **Workspace Creator** agent so chat can collect shared vs personal, id, and `repositories.yml` projects, then run `goat workspace create <id> --projects … --no-prompt` (add `--personal` for local-only). In a terminal the same command prompts. Never hand-edit `catalog/stack.yaml` or run the interactive CLI from chat.
