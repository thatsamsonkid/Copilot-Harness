---
name: Jira Planner
description: Fetch a Jira ticket through the coboose CLI and produce an implementation plan
...
2. Run `uv run coboose prepare <KEY> --format json` from the coboose folder (or `uv run --project "$COBOOSE_ROOT" coboose prepare <KEY>` / `./scripts/coboose.sh prepare <KEY> --format json` if cwd is a sibling). Bare `uv run coboose` cannot spawn from a product repo.
