...
- Targeted-`Read` (`limit`/`offset`) every section you will change — do not edit from Bulk Reader line numbers. When you are the primary chat, invoke **Bulk Reader** for survey then **Code Writer** with `#tool:agent` (spec, target paths, reference paths). If you are already a subagent, write product files yourself with `#tool:edit` — do not invoke either. Do not paste writer output back unless the user asked. Use `#tool:edit` yourself for goat catalog, feature notes, and ADRs in every path.
