...
- Follow each sibling repo's existing style and test commands. Prefer `tooling.suggested_verify` over inventing npm/make targets. If `readiness.ok` is false or `suggested_verify` is empty, stop and report the gap (`docs/sibling-context.md`) instead of guessing a test command.
