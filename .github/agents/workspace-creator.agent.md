...
- Run `uv run harness repos --format json` and `uv run harness workspace list --format json` from the harness folder first (or `uv run --project "$HARNESS_ROOT" harness …` / `./scripts/harness.sh` after `./scripts/setup.sh`). Do not `cd` into a sibling first.
