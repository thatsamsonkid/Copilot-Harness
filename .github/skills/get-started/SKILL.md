...
10. From init JSON `workspaces[]`, list the catalog starters and ask which to open (`open_command` / `coboose workspace open <id>`). They can also create their own with `/new-workspace` or `coboose workspace create` (adds an id to the catalog).
