...
description: Discover Graphify graphs, instruction files, and verify commands in sibling repos. Use for vague or low-context prompts, large monorepos, "where does X live?", or before implementing in a product repo. Prefer graphify query/path/explain and local AGENTS.md over grepping the whole tree.
---
...
| Ticket plus routing | `uv run harness prepare <KEY> --format json` (each `routing.repos[]` already includes `graphify`, `instructions`, `tooling`) |

## Vague or low-context prompts

1. Run `harness context` (or use `prepare` if a Jira key is present).
2. For each cloned repo with `graphify.present`, read `graphify.report` (`GRAPH_REPORT.md`) before opening source files.
