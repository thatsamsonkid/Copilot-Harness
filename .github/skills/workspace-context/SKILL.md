...
Product code lives in clones next to this coboose (flat siblings or grouped folders such as `frontend/shop-web`). Those repos may already have Graphify output and their own Copilot instructions. The coboose only discovers them.

## Commands

Run these from the coboose repo. After `cd` into a sibling, use `uv run --project "$COBOOSE_ROOT" coboose …` or `./scripts/coboose.sh` — bare `uv run coboose` cannot spawn from a product clone.
...
The coboose does not own product patterns.
...
- Do not copy those files into the coboose. Do not invent style rules that contradict them.
- Product knowledge stays in the sibling (`docs/features`, ADRs). If you added user-visible or non-obvious behavior, update or create a note there using `templates/feature-note.md`. Do not start a wiki in the coboose.
