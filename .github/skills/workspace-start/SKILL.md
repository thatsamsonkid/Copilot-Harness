...
description: Plan and start local apps in the current feature workspace (Java, Angular, Node). Use when the user wants to run the stack, start services, wire Angular proxies to local backends, or asks how to boot every repo. Discover with goat start; start one process at a time. Do not launch everything in parallel. Never read launch.json or reconstruct env/args.
---

# Workspace start

The goat does **not** own product start scripts. `goat start` inspects sibling clones and prints a plan. You start processes one at a time so real ports and proxy rewrites can be applied after each backend is up.

A workspace start sequence rarely changes. After the first good plan, save it next to the workspace file as `workspaces/<id>.start.yml`. Later `goat start --workspace <id>` prefers that file over rediscovery. Live clone status, proxy files, and compose markers are still inspected. Do not put `start:` on `repositories.yml` entries.

Java apps often keep args and environment variables in `.vscode/launch.json`. Those values must not enter chat. The plan only lists configuration names and env **keys**. Start those apps with `goat start run` or VS Code **Run Without Debugging** — not by reading `launch.json` and not by forcing Debug.

## Commands

| User intent | Command |
| --- | --- |
| Plan for the open feature workspace | `uv run goat start --format json` (uses `GOAT_WORKSPACE`) |
| Plan for one catalog workspace | `uv run goat start --workspace <id> --format json` |
| Plan for every enabled repo | `uv run goat start --all --format json` (only if the user asked) |
| Pin the current sequence next to the workspace | `uv run goat start --save --format json` |
| Rediscover and overwrite the saved plan | `uv run goat start --workspace <id> --refresh --save --format json` |
| One or more repos | `uv run goat start --repo frontend,backend --format json` |
| Human-readable plan | `uv run goat start --workspace <id> --format markdown` |
| Start one repo with launch.json env/args (no leak) | `uv run goat start run --repo <name>` |
| Preview that run (keys + redacted `exec_command`) | `uv run goat start run --repo <name> --dry-run --format json` |
| Inspect one repo's launch env (keys + collisions) | `uv run goat start env --repo <name> --format json` |
| Apply that env in a dedicated terminal | `uv run goat start env --repo <name> --shell` |

`goat start` (no `run`) never launches processes. Do not invent a second supervisor. `--save` writes the full workspace sequence (needs `--workspace` or `GOAT_WORKSPACE`); do not combine it with `--repo`. Stay inside `workspace.repos`. Do not start clones that are only on disk.
...
`uv run goat` only works when the process **cwd is this goat repo** (`pyproject.toml` / `uv.lock`). Sibling product clones do not have that project. If you `cd` into a repo (or set the terminal/`runCommands` cwd there) uv fails with **Failed to spawn: `goat`**.

Always do one of:

1. Keep cwd on the goat root (first workspace folder, named `goat`). Do not `cd` before this command.
2. `uv run --project "$GOAT_ROOT" goat start --format json` from any cwd (same `--project` form for `start run`)
3. `./scripts/goat.sh start --format json` or `.\scripts\goat.ps1 start --format json` using the goat-root path

`goat start` JSON includes `invoke.command`, `invoke.cwd`, and `invoke.script` for the cwd-safe form. App terminals stay in each service `path`. Never run `uv run goat` from those terminals.

## Walkthrough order

1. Run `goat start` for the workspace the user has open (or `--repo` if they named apps). Use the goat cwd or `invoke.command` — do not `cd` into a product repo first. If `workspace_scope.detected` is false, pass `--workspace <id>` or ask which feature workspace is open. Never plan every `parent_dir` clone just because they exist.
...
| `run_via` | What to do |
| --- | --- |
| `goat` | Run `copilot_command` from the goat cwd (it already includes `--project`). Do not `cd` into the sibling first — the CLI starts the process in the repo cwd and applies launch env to that child only. Bare `uv run goat` cannot spawn from an app terminal. The CLI loads launch.json / envFile in-process and does not print values. If you need the env in the terminal itself (without starting the app), use `start env --repo <name> --shell` |
...
The goat does not open terminals. You do, via the chat terminal / `runCommands` tool.
...
- Leave those terminals running. Do not reuse a busy app terminal to run `goat start` or git commands. `uv run goat` cannot spawn from a product-repo cwd.

## Hard rules

- Stay in listed clone folders. Never nest git clones inside the goat.
...
- Never read sibling `.vscode/launch.json`, `envFile`, or product `.env` files. `goat start` already redacts them to names and keys.
- Never `export` launch env, reconstruct args/vmArgs, or paste secrets into a terminal command. Use `goat start env --repo <name>` (keys and collisions only) or `goat start env --repo <name> --shell` (applies values in-process to a new shell). Do not `eval` or print values.
- Application env keys stay unprefixed so Spring/Node see the same names as VS Code. Collisions with the current terminal are reported as `overwritten_keys`. Pass `--keep-existing` to leave already-set keys alone. Use `--prefix BACKEND` only when you want namespaced copies (`BACKEND_DB_PASSWORD`); that will not satisfy an app looking up `DB_PASSWORD`. Goat stamps `GOAT_ENV_REPO` (and `GOAT_ENV_CONFIGURATION` when known) so you can see which project env is active.
...
| Symptom | What to do |
| --- | --- |
| `blocked: repo is not cloned` | Show `goat clone --only <name>` (or `prepare` clone command). Do not `git clone` into the goat folder |
...
| `uses_vscode_inputs` | Same as vscode. `goat start run` / `start env` cannot resolve `${input:…}` |
...
| User wants this sequence next time | `goat start --workspace <id> --save` → `workspaces/<id>.start.yml` |
...
| `Failed to spawn: goat` / no `pyproject.toml` | Cwd is a sibling clone. Re-run from the goat folder, or use `invoke.command` / `scripts/goat.sh` (Windows: `scripts/goat.ps1`) |
...
This file is the boot **sequence** (order, command, port, wait, optional `launch` / `method`). It is not a process supervisor. When discovery is wrong, edit this file. `launch` selects a `launch.json` configuration; `method` is `terminal`, `vscode`, or `goat`. Prefer keeping secrets in a gitignored `envFile` / `.env` referenced from `launch.json`.
