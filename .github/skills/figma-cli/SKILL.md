...
| User intent | Command |
| --- | --- |
| Export rendered frame URLs (default) | `uv run goat figma images <URL>` |
| File key plus node ids | `uv run goat figma images <KEY> --ids 12:34,12:56` |
| Override format / scale | add `--image-format png` and/or `--scale 2` |
| Design comments on the pasted frame | `uv run goat figma comments <URL>` |
| Comments for the whole file | add `--file-comments` |
| Raw node JSON for a small targeted frame | `uv run goat figma nodes <URL>` |
| Shallower / deeper node tree | add `--depth 1` (max is `figma.max_depth`) |
| What Copilot is allowed to see | `uv run goat figma schema` |
| Auth check (no token in output) | `uv run goat figma whoami` |
| Store token in OS keychain | Tell them to run `uv run goat figma login` (or `--from-env`) themselves |
| Catalog / clones / env | `uv run goat doctor` |
| Live Figma ping | `uv run goat doctor --ping-figma` |
...
| Symptom | What to do |
| --- | --- |
| Missing `FIGMA_ACCESS_TOKEN` | Tell the user to run `uv run goat figma login` (`docs/figma-access-token.md`) |
| 401 / 403 from the CLI | Tell them to rotate the Figma personal access token and run `uv run goat figma login` |
