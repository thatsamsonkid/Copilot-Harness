...
| Lift coboose + sibling skills into the root | `uv run coboose skills lift --format json` |
| Lift a subset | `uv run coboose skills lift --only <pick,pick>` |
| Preview a lift | `uv run coboose skills lift --dry-run` |
...
`init`, `prepare`, `workspace generate`, `workspace create`, and `doctor` already run `skills lift` for coboose plus in-scope siblings. Use the commands above when the user wants a subset, a refresh, or a remote repo.

## Walkthrough

1. Run `coboose skills list`. Stay inside `workspace.repos` unless they asked for `--all`.
2. Show `available[]` (`pick`, `name`, `source_id`, `description`) and `installed[]`. First-party coboose skills are already in the dest; they are not copied over themselves.
3. If they want sibling skills in the Agents window, run `coboose skills lift` (or `--only` with `available[].pick`).
