...
class UnavailableStore:
    ...
    def set_password(self, service: str, username: str, password: str) -> None:
        raise GoatError(_unavailable_message())
...
def set_stored_secret(account: str, value: str) -> None:
    ...
    if not cleaned:
        raise GoatError("A non-empty secret is required")
    if not account.strip():
        raise GoatError("A keychain account name is required")
...
def delete_stored_secret(account: str) -> bool:
    ...
    for service in (SERVICE, LEGACY_SERVICE):
        try:
            ...
            last_error = exc
    if get_stored_secret(account) is not None:
        raise GoatError(
            f"Could not remove the secret from {backend_display_name()}: {last_error}."
        ) from last_error
    return True
...
def storage_guides() -> dict[str, Any]:
    ...
    return {
        "preferred_cli": "uv run goat jira login",
        "env_command": "uv run goat env set NAME",
        "migrate_command": "uv run goat jira login --from-env",
        "service": SERVICE,
        "account": ACCOUNT,
...
def storage_guide(system: str | None = None) -> dict[str, Any]:
    ...
    if system == "Darwin":
        return {
            ...
            "cli_command": "uv run goat jira login",
            "migrate_command": "uv run goat jira login --from-env",
            "service": SERVICE,
            "account": ACCOUNT,
            "manual_steps": [
    ...
    return {
        ...
        "account": ACCOUNT,
        "manual_steps": [
            "On a desktop session with GNOME Keyring or KWallet, run `uv run goat jira login`.",
    ...
    if from_env:
        ...
        if not token:
            raise GoatError(
                ...
                "Run `uv run goat jira login` in your own terminal, "
                f"or see {TOKEN_DOC}."
            )
    else:
        ...
        if not stdin_isatty:
            raise GoatError(
                ...
                "terminal, or use `goat jira login --from-env` if the token is "
        ...
        if not token:
            raise GoatError("A non-empty API token is required")

    set_stored_token(token)
    cleared_env = False
    if clear_env:
        cleared_env = _clear_env_token(goat_root)
...
def logout_token(goat_root: Path, *, clear_env: bool = False) -> dict[str, Any]:
    removed = delete_stored_token()
    cleared_env = _clear_env_token(goat_root) if clear_env else False
    status = keychain_status()
    return {
        "removed": removed,
        "cleared_env": cleared_env,
...
def missing_token_action() -> str:
    return (
        ...
        "`uv run goat jira login` (macOS Keychain or Windows Credential Manager). "
