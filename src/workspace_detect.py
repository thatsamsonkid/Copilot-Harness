...
from goat.catalog import Catalog, Repo, Workspace, workspace_document_meta

WORKSPACE_ID_ENV = "GOAT_WORKSPACE"
WORKSPACE_FILE_ENV = "GOAT_WORKSPACE_FILE"
LEGACY_WORKSPACE_ID_ENV = "COBOOSE_WORKSPACE"
LEGACY_WORKSPACE_FILE_ENV = "COBOOSE_WORKSPACE_FILE"
...
def resolve_workspace_scope(
    ...
    *,
    ...
    ``GOAT_WORKSPACE`` / ``GOAT_WORKSPACE_FILE``. When nothing is
    detected, the scope is every enabled repo so CI and a single-folder
    Goat window keep working.
    """
    if all_repos and workspace_id:
        raise GoatError("Do not combine --workspace with --all.")
    ...
    env_id = _clean_env(env.get(WORKSPACE_ID_ENV)) or _clean_env(
        env.get(LEGACY_WORKSPACE_ID_ENV)
    )
    if env_id:
        source_name = (
            WORKSPACE_ID_ENV
            if _clean_env(env.get(WORKSPACE_ID_ENV))
            else LEGACY_WORKSPACE_ID_ENV
        )
        return _scope_for_workspace(
            catalog,
            goat_root,
            ...
            source="env",
            detail=(
                f"Using workspace {env_id} from {source_name}. "
                "Stay inside these repos unless the user asked for --all."
            ),
        )

    env_file = _clean_env(env.get(WORKSPACE_FILE_ENV)) or _clean_env(
        env.get(LEGACY_WORKSPACE_FILE_ENV)
    )
...
def scoped_repos(
    ...
    *,
    ...
    if only:
        ...
        if unknown:
            raise GoatError("Unknown repo name(s): " + ", ".join(sorted(unknown)))
...
def _scope_from_file(
    ...
    *,
    ...
    if not resolved.exists():
        raise GoatError(f"Workspace file missing: {resolved}")
    resolved = resolved.resolve()
    matched = _workspace_matching_file(catalog, goat_root, resolved)
    if matched is None:
        raise GoatError(
...
def _workspace_matching_file(
    catalog: Catalog, goat_root: Path, path: Path
) -> Workspace | None:
    for workspace in catalog.workspaces:
        candidate = catalog.workspace_file(goat_root, workspace)
        try:
