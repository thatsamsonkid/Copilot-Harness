...
def write_handoff(
    ...
    *,
    ...
    directory = _handoff_dir(harness_root)
...
def list_handoffs(harness_root: Path) -> list[dict[str, Any]]:
    directory = _handoff_dir(harness_root)
    if not directory.is_dir():
        return []
    items = []
    for path in sorted(directory.glob("*.md"), reverse=True):
        items.append(_summarize(path, harness_root))
    return items


def latest_handoff(harness_root: Path) -> dict[str, Any]:
    items = list_handoffs(harness_root)
    if not items:
        raise HarnessError("No handoff notes yet. Run harness handoff write.")
    latest = items[0]
...
def _handoff_dir(harness_root: Path) -> Path:
    return harness_root / HANDOFF_DIR
...
def _summarize(path: Path, harness_root: Path) -> dict[str, Any]:
    issue = None
    title = path.stem
    ...
    *,
    ...
    lines.extend(
        [
            ...
            "2. Run `uv run harness status --format json` to refresh the snapshot.",
            "3. If there is a Jira key, run `uv run harness prepare <KEY> --format json`.",
            "",
        ]
    )
    return "\n".join(lines)
