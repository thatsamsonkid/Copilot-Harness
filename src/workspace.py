...
def workspace_document(catalog: Catalog, goat_root: Path, workspace: Workspace) -> dict[str, Any]:
    ...
    if workspace.include_goat:
        folders.append(
            {
                "name": GOAT_FOLDER_NAME,
                "path": _rel(workspace_file, goat_root),
            }
        )
    for repo_id in catalog.workspace_repo_names(workspace):
        ...
        folders.append(
            {
                "name": repo.name,
                "path": _rel(workspace_file, catalog.repo_path(goat_root, repo)),
            }
        )
    ...
    settings.update(
        terminal_env_settings(
            folder_name=GOAT_FOLDER_NAME if workspace.include_goat else None,
            workspace_id=workspace.id,
            include_root=workspace.include_goat,
        )
    )
    return {
        "folders": folders,
...
def generate_workspaces(catalog: Catalog, goat_root: Path) -> list[dict[str, Any]]:
    return [
...
def list_workspaces(catalog: Catalog, goat_root: Path) -> list[dict[str, Any]]:
    ...
    for workspace in catalog.workspaces:
        path = catalog.workspace_file(goat_root, workspace)
        start_file = catalog.workspace_start_file(goat_root, workspace)
        repos = []
        for repo_id in catalog.workspace_repo_names(workspace):
            repo_path = catalog.repo_path(goat_root, repo_id)
            repos.append(
                {
                    "id": repo_id,
                    "path": str(repo_path),
...
def open_workspace(workspace_file: Path) -> dict[str, Any]:
    if not workspace_file.exists():
        raise GoatError(
            f"Workspace file missing: {workspace_file}. Run `goat workspace generate`."
        )
