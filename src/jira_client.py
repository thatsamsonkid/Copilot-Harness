...
from goat.keychain import resolve_token
...
def parse_issue_key(value: str) -> str:
    text = (value or "").strip()
    if not text:
        raise GoatError("Issue key is required")
    if re.fullmatch(r"[A-Z][A-Z0-9]+-\d+", text):
        return text
    match = ISSUE_KEY_RE.search(text)
    if match:
        return match.group(1)
    raise GoatError(f"Could not parse a Jira issue key from: {value}")
...
def _missing_settings_message(missing: list[str]) -> str:
    ...
    if "JIRA_API_TOKEN" in missing:
        parts.append(
            "Store the API token with `uv run goat jira login` "
...
class JiraClient:
    ...
    def search(
        ...
        if not jql.strip():
            raise GoatError("JQL is required")
        settings = settings or JiraSettings()
    ...
    def _decode(self, response: HttpResponse, method: str) -> Any:
        if response.status == 401:
            raise GoatError(
                ...
                "(`goat jira login`) or .env; do not print them."
            )
        if response.status == 403:
            raise GoatError("Jira denied access to this resource.")
        if response.status == 404:
            raise GoatError("Jira resource not found.")
        if response.status >= 400:
            detail = _error_message(response)
            raise GoatError(f"Jira API {method} failed ({response.status}): {detail}")
        try:
            return response.json()
        except ValueError as exc:
            raise GoatError("Jira returned a non-JSON response") from exc
