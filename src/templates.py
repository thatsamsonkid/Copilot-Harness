...
from goat.catalog import Repo, as_list, read_yaml
from goat.paths import TEMPLATES_RELATIVE
...
def load_templates(path: Path) -> list[Template]:
    ...
    if raw is None:
        ...
        raise GoatError(f"{path} must be a mapping or a list of templates")
    ...
    for item in items:
        ...
        if template.name in seen:
            raise GoatError(f"Duplicate template name: {template.name}")
...
def _parse_template(item: Any) -> Template:
    if not isinstance(item, dict):
        raise GoatError("Each template entry must be a mapping")
    name = item.get("name") or item.get("id")
    url = item.get("url") or item.get("clone_url") or item.get("git")
    if not name or not url:
        raise GoatError("Each template needs name and url (GitHub clone URL)")
    name = str(name)
    tags = as_list(item.get("tags"))
    if not tags:
        raise GoatError(f"Template {name} needs at least one tag")
    return Template(
        name=name,
        url=str(url),
        tags=tags,
    ...
    if only:
        ...
        if unknown:
            raise GoatError(
                ...
                + ", ".join(sorted(unknown))
                + ". Run `goat templates` to see the list."
            )
...
def get_template(templates: list[Template], name: str) -> Template:
    for item in templates:
        if item.name == name:
            return item
    raise GoatError(
        f"Unknown template: {name}. Run `goat templates` to see the list."
    )
