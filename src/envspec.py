...
from goat.paths import ENV_RELATIVE
...
def load_env_spec(path: Path | None) -> tuple[list[EnvVar], Path | None]:
    ...
    from goat.catalog import read_yaml

    raw = read_yaml(path)
    if raw is None:
        return default_env_vars(), path
    if not isinstance(raw, dict):
        raise GoatError(f"{path} must be a mapping with a variables list")
    items = raw.get("variables")
    if items is None:
        return default_env_vars(), path
    if not isinstance(items, list):
        raise GoatError(f"{path} variables must be a list")
    ...
    for item in items:
        ...
        for name in variable.names:
            if name in seen:
                raise GoatError(f"{path} has a duplicate env name: {name}")
            seen.add(name)
        variables.append(variable)
    if not variables:
        raise GoatError(f"{path} variables list is empty")
...
def missing_action(variable: EnvVar) -> str:
    ...
    if variable.secret:
        command = (
            ...
            if variable.name == "JIRA_API_TOKEN"
            else "uv run goat figma login"
            if variable.name == "FIGMA_ACCESS_TOKEN"
            else f"uv run goat env set {variable.name}"
        )
        return (
...
def list_env(
    ...
    *,
    ...
    file_keys = env_file_keys(goat_root / ".env")
    ...
    if from_env:
        ...
        if source != SOURCE_ENV or not value:
            raise GoatError(
                ...
                f"Run `uv run goat env set {variable.name}` in your own terminal"
        ...
        if not stdin_isatty:
            raise GoatError(
                ...
                f"terminal, or use `goat env set {variable.name} --from-env` "
...
def unset_env_value(
    ...
    *,
    ...
    if clear_env or not variable.secret:
        cleared_env = _clear_env_names(goat_root / ".env", variable.names)
    return {
        "removed": removed,
        "cleared_env": cleared_env,
        **var_status(variable, env_file_keys(goat_root / ".env")),
        "keychain_guide": storage_guides(),
    }
...
def _prompt_value(
    ...
    *,
    ...
    if not value:
        raise GoatError(f"A non-empty value is required for {variable.name}")
    return value
...
def _parse_var(item: Any, source: Path) -> EnvVar:
    ...
    if not isinstance(item, dict):
        raise GoatError(f"{source} each variable must be a mapping or a name")
