...
def _load_bruno(raw: Any, repo_names: set[str]) -> BrunoSettings:
    data = raw or {}
    if not isinstance(data, dict):
        raise CobooseError("bruno settings must be a mapping")
...
def _parse_bruno_services(raw: Any, repo_names: set[str]) -> list[BrunoService]:
    ...
    if isinstance(raw, list):
        ...
        raise CobooseError("bruno.services must be a list or a mapping")
    services: list[BrunoService] = []
    seen: set[str] = set()
    for item in items:
        if not isinstance(item, dict):
            raise CobooseError("Each bruno.services entry must be a mapping")
        service_id = str(item.get("id") or item.get("name") or "").strip()
        if not service_id:
            raise CobooseError("Each bruno.services entry needs an id")
        if service_id in seen:
            raise CobooseError(f"Duplicate bruno.services id: {service_id}")
        seen.add(service_id)
