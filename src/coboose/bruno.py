...
Coboose does not reimplement HTTP. ``bru run`` already executes a request
...
from coboose.bruno_fields import (
    DEFAULT_ENV,
...
def collect_bruno_inventory(
    ...
    *,
    ...
    discovered = _discover(catalog, coboose_root, settings)
    payload = {
    ...
    if collection is not None:
        ...
        if not requests:
            raise CobooseError(
                ...
                "Run `coboose bruno collections` or `coboose bruno requests`."
            )
    else:
    ...
    *,
    ...
    payload = {
        ...
        "note": (
            ...
            "or `coboose bruno run REQUEST --env NAME`. Secrets stay in "
    ...
    *,
    ...
    payload = {
        ...
        "note": (
            ...
            "with `coboose bruno run` (or `bru run` from the collection "
...
def run_bruno_request(
    ...
    *,
    ...
    if owner is None:
        raise CobooseError(f"Request {target!r} has no collection")
    ...
    if not bru:
        raise CobooseError(
            ...
            "Discovery (`coboose bruno collections`) does not need bru."
        )
    ...
    if payload["exit_code"] != 0:
        raise CobooseError("bru run failed", payload=projected)
    return projected
...
def parse_env_vars(values: Iterable[str] | None) -> list[tuple[str, str]]:
    ...
    for raw in values or []:
        ...
        key = key.strip()
        if not key:
            raise CobooseError(f"Invalid --env-var {raw!r}. Use KEY=value.")
...
def _discover(
    catalog: Catalog, coboose_root: Path, settings: BrunoSettings
) -> dict[str, Any]:
    bru = bru_cli_status()
...
def _scan_repo(repo: Repo, root: Path, settings: BrunoSettings) -> list[BrunoCollection]:
    ...
    repo_workflows = parse_workflow_file(
        root / settings.workflows_file, repo=repo.name
    )
    repo_services = parse_services_file(
        root / settings.services_file, repo=repo.name
    )
...
def _load_collection(
    ...
    workflows = parse_workflow_file(
        collection_root / settings.workflows_file,
        collection_id=collection_id,
        repo=repo.name,
    )
    services = parse_services_file(
        collection_root / settings.services_file,
        collection_id=collection_id,
        repo=repo.name,
    )
...
def _require_service(services: list[dict[str, Any]], name: str) -> dict[str, Any]:
    ...
    if len(matches) != 1:
        ...
        raise CobooseError(f"Unknown Bruno service {name!r}. Known: {known}")
    return matches[0]
...
def _require_request(
    ...
    if not matches:
        raise CobooseError(
            ...
            "Run `coboose bruno requests` for ids and paths."
        )
