    ...
    def from_mapping(
        ...
        *,
        ...
        if not isinstance(shapes_raw, Mapping):
            raise CobooseError(f"{label}shapes must be a mapping of field to nested keys")
...
def _as_str_list(value: Any, *, label: str) -> list[str]:
    ...
    raise CobooseError(f"Expected {label} to be a list or string, got {type(value).__name__}")
