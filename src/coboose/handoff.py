...
def write_handoff(
    ...
    *,
    ...
    directory = _handoff_dir(coboose_root)
    ...
    return {
        ...
        "issue": issue,
        "written_at": stamp,
        "relative": str(path.relative_to(coboose_root)),
    }


def list_handoffs(coboose_root: Path) -> list[dict[str, Any]]:
    directory = _handoff_dir(coboose_root)
    if not directory.is_dir():
        return []
    items = []
    for path in sorted(directory.glob("*.md"), reverse=True):
        items.append(_summarize(path, coboose_root))
    return items


def latest_handoff(coboose_root: Path) -> dict[str, Any]:
    items = list_handoffs(coboose_root)
    if not items:
        raise CobooseError("No handoff notes yet. Run coboose handoff write.")
    latest = items[0]
...
def _handoff_dir(coboose_root: Path) -> Path:
    return coboose_root / HANDOFF_DIR
...
def _summarize(path: Path, coboose_root: Path) -> dict[str, Any]:
    issue = None
    title = path.stem
    ...
    *,
    ...
    lines.extend(
        [
            ...
            "2. Run `uv run coboose status --format json` to refresh the snapshot.",
            "3. If there is a Jira key, run `uv run coboose prepare <KEY> --format json`.",
            "",
        ]
    )
    return "\n".join(lines)
