...
Writing an implementation plan to a file (for another or a smaller model to execute): `/goat-plan` or `.github/skills/planning/SKILL.md`. Plans go in the root `plans/` directory (gitignored), start from `templates/plan.md`, and must be detailed enough for a zero-context executor — a complete file map of every file to change, plus exact symbols, commands, expected results, and verify checks per step. Start implementation with `/goat-implement` or `.github/skills/implementing/SKILL.md` — that chat becomes Implementer, invokes Code Writer and Verifier, and must not spawn Implementer. After compaction, run `/goat-implement` rather than a bare "implement it". The `implement-gate` hook denies parent product-file edits while that command is armed. A later new chat that only has the plan file and did not run `/goat-implement` writes the code itself and must not spawn Implementer, Code Writer, or Verifier.
