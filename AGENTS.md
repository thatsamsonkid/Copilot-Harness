...
Writing an implementation plan to a file (for another or a smaller model to execute): `/goat-plan` or `.github/skills/planning/SKILL.md`. Plans go in the root `plans/` directory (gitignored), start from `templates/plan.md`, and must be detailed enough for a zero-context executor — a complete file map of every file to change, plus exact symbols, commands, expected results, and verify checks per step. Do not write a plan (chat or `plans/`) for small direct tasks — implement those. Chat plans are only for `/jira-ticket` / Jira Planner or when the user asked to plan; file plans are only `/goat-plan`. A later new chat that only has that file writes the code itself and must not spawn Implementer or Code Writer. The same chat that wrote the plan becomes Implementer and invokes Code Writer — it must not spawn Implementer.
...
Then open `routing.open_command` and work against those roots. Treat `done_when` as the stop condition. Do not nest git clones inside this repository. Do not write an implementation plan unless they asked, invoked `/jira-ticket` / Jira Planner, or invoked `/goat-plan`.
