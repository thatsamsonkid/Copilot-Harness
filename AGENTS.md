...
Run `uv run coboose …` from this Coboose repo (or `uv run --project "$COBOOSE_ROOT" coboose …` / `./scripts/coboose.sh`). After `cd` into a sibling clone, bare `uv run coboose` cannot spawn. Full command catalog: `uv run coboose commands --format json` or [docs/cli.md](docs/cli.md).
