...
Run `uv run coboose …` from this Coboose repo (or `uv run --project "$COBOOSE_ROOT" coboose …` / `./scripts/coboose.sh`). After `cd` into a sibling clone, bare `uv run coboose` cannot spawn. The old `harness` command still works as an alias.
