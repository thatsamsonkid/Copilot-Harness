...
**Jira Planner** (and `/jira-ticket`) orchestrates automatically: it invokes the hidden **Bulk Reader** subagent for product files so the plan chat stays a map. After `/goat-plan`, never spawn Implementer. Plans group independent steps into **Parallel waves**. Start implementation with **`/goat-implement`** — that command survives chat compaction: the chat becomes Implementer, fans out **Code Writer** per independent step in a wave, then **Verifier** for that wave (it still runs `goat branch` and feature notes). A new chat with only the `plans/` file and no `/goat-implement` (often a smaller model) is the executor and writes the files itself. VS Code Agents dropdown (pick **Implementer** as primary) is the same as `/goat-implement`. Subagents must not spawn other subagents. Do not pick Orchestrator — it is not in the agents dropdown.
