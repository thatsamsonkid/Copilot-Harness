...
from harness.workspace_create import (
    create_workspace,
    format_project_menu,
...
def test_slugify_and_validate():
    ...
    with pytest.raises(HarnessError, match="slug"):
        validate_workspace_id("***")
...
def test_parse_project_selection_numbers_names_tags_and_all(catalog):
    ...
    with pytest.raises(HarnessError, match="Unknown project"):
        parse_project_selection("missing", repos)
    with pytest.raises(HarnessError, match="out of range"):
        parse_project_selection("9", repos)
    with pytest.raises(HarnessError, match="No repositories.yml entry has tag"):
...
def test_create_workspace_with_flags(catalog, harness_root: Path):
    ...
    assert payload["folders"] == ["harness", "frontend", "backend"]
    path = harness_root / "workspaces" / "checkout.code-workspace"
    assert path.exists()
...
def test_create_workspace_requires_flags_without_tty(catalog, harness_root: Path):
    with pytest.raises(HarnessError, match="--id"):
        create_workspace(
            catalog,
            harness_root,
            prompt=PromptSession(interactive=False),
        )
    with pytest.raises(HarnessError, match="--projects"):
        create_workspace(
            catalog,
            harness_root,
            workspace_id="checkout",
...
def test_create_workspace_rejects_unknown_and_duplicate(catalog, harness_root: Path):
...
def test_create_workspace_force_replaces(catalog, harness_root: Path):
    payload = create_workspace(
        catalog,
        harness_root,
        workspace_id="frontend",
...
def test_create_workspace_dry_run_does_not_write(catalog, harness_root: Path):
    payload = create_workspace(
        catalog,
        harness_root,
        workspace_id="checkout",
    ...
    assert not (harness_root / "workspaces" / "checkout.code-workspace").exists()
    with pytest.raises(HarnessError, match="Unknown workspace"):
        load_catalog(harness_root).workspace("checkout")


def test_create_workspace_prompts_for_projects(catalog, harness_root: Path):
    stdin = io.StringIO("\ncheckout\nCheckout\nCart flow\n1,2\n\n")
    stderr = io.StringIO()
    payload = create_workspace(
        catalog,
    ...
    assert load_catalog(harness_root).workspace("checkout").folders == [
        "frontend",
        "backend",
    ]


def test_create_workspace_reprompts_invalid_selection(catalog, harness_root: Path):
    stdin = io.StringIO("\npayments\n\n\nnope\n2\ny\n")
    stderr = io.StringIO()
    payload = create_workspace(
        catalog,
    ...
    assert payload["workspace"]["include_harness"] is True


def test_create_workspace_cli(harness_root: Path, capsys, monkeypatch):
    ...
    assert payload["created"] is True
    assert payload["workspace"]["id"] == "checkout"
    assert (harness_root / "workspaces" / "checkout.code-workspace").exists()
...
def test_create_personal_workspace_skips_catalog(catalog, harness_root: Path):
    stack_before = (harness_root / "catalog" / "stack.yaml").read_text(encoding="utf-8")
    payload = create_workspace(
        catalog,
        harness_root,
    ...
    path = harness_root / "workspaces" / "personal" / "scratch.code-workspace"
    assert path.exists()
    assert payload["workspace"]["file"] == str(path)
    assert not (harness_root / "workspaces" / "scratch.code-workspace").exists()
    assert (harness_root / "catalog" / "stack.yaml").read_text(
        encoding="utf-8"
    ) == stack_before

    document = json.loads(path.read_text(encoding="utf-8"))
    assert document["harness"]["personal"] is True
    assert document["harness"]["folders"] == ["frontend", "backend"]
    assert [folder["name"] for folder in document["folders"]] == [
        "harness",
        "frontend",
        "backend",
...
def test_create_personal_workspace_prompts_for_kind(catalog, harness_root: Path):
    ...
    assert (harness_root / "workspaces" / "personal" / "scratch.code-workspace").exists()
    stack_ids = [
        item["id"]
        for item in yaml.safe_load(
            (harness_root / "catalog" / "stack.yaml").read_text(encoding="utf-8")
        ).get("workspaces")
        or []
    ]
...
def test_create_personal_rejects_shared_id_and_fallback(catalog, harness_root: Path):
    with pytest.raises(HarnessError, match="already exists as a shared"):
        create_workspace(
            catalog,
            harness_root,
            workspace_id="frontend",
    ...
    with pytest.raises(HarnessError, match="cannot be fallback"):
        create_workspace(
            catalog,
            harness_root,
            workspace_id="scratch",
    ...
    with pytest.raises(HarnessError, match="only exist as .code-workspace"):
        create_workspace(
            catalog,
            harness_root,
            workspace_id="scratch",
...
def test_create_personal_force_replaces(catalog, harness_root: Path):
    create_workspace(
        catalog,
        harness_root,
        workspace_id="scratch",
    ...
    assert payload["replaced"] is True
    document = json.loads(
        (harness_root / "workspaces" / "personal" / "scratch.code-workspace").read_text(
            encoding="utf-8"
        )
    )
    assert document["harness"]["folders"] == ["backend"]


def test_create_personal_cli(harness_root: Path, capsys, monkeypatch):
    monkeypatch.chdir(harness_root)
    assert (
        main(
            [
