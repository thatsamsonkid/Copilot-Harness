...
from goat.templates import get_template, load_templates, select_templates
from tests.helpers import write_goat_config, write_yaml


def test_load_templates_from_goat_root(catalog):
...
def test_rejects_duplicate_template_names(tmp_path: Path):
    ...
    with pytest.raises(GoatError, match="Duplicate template"):
        load_templates(path)
...
def test_select_templates_by_tag_and_unknown_name(catalog):
    ...
    with pytest.raises(GoatError, match="Unknown template"):
...
def test_rejects_repositories_key_in_templates_file(tmp_path: Path):
    ...
    with pytest.raises(GoatError, match="repositories.yml"):
        load_templates(path)
