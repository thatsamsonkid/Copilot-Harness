...
from coboose.catalog import load_catalog, load_repositories
from coboose.clone import clone_repos
from tests.helpers import write_coboose_config, write_yaml


def test_repositories_yml_is_the_manifest(coboose_root: Path):
    catalog = load_catalog(coboose_root)
    assert catalog.repos_source == coboose_root / "repositories.yml"
...
def test_rejects_repos_key_in_stack(tmp_path: Path, sample_catalog_data: dict):
    ...
    with pytest.raises(CobooseError, match="must not list repositories"):
        load_catalog(root)


def test_clone_filters_by_tag(catalog, coboose_root: Path):
    result = clone_repos(catalog, coboose_root, tags=["ui"], dry_run=True)
