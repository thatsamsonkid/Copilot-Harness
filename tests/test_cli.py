...
def test_figma_schema_needs_no_credentials(goat_root: Path, capsys, monkeypatch):
    monkeypatch.chdir(goat_root)
    monkeypatch.delenv("FIGMA_ACCESS_TOKEN", raising=False)
    assert main(["--root", str(goat_root), "figma", "schema"]) == 0
...
def test_figma_images_uses_client(goat_root: Path, capsys, monkeypatch):
    monkeypatch.chdir(goat_root)
    ...
    monkeypatch.setattr("goat.cli._figma_client", lambda _catalog: FakeClient())
    assert main(
        [
            "--root",
            str(goat_root),
...
def test_figma_comments_and_nodes_use_client(goat_root: Path, capsys, monkeypatch):
    monkeypatch.chdir(goat_root)
    monkeypatch.setenv("FIGMA_ACCESS_TOKEN", "figd_test")
...
def test_missing_figma_env(goat_root: Path, capsys, monkeypatch):
    monkeypatch.chdir(goat_root)
    monkeypatch.delenv("FIGMA_ACCESS_TOKEN", raising=False)
    monkeypatch.delenv("FIGMA_TOKEN", raising=False)
    monkeypatch.delenv("FIGMA_API_TOKEN", raising=False)
    assert main(["--root", str(goat_root), "figma", "whoami"]) == 1
    error = json.loads(capsys.readouterr().err)
    assert "FIGMA_ACCESS_TOKEN" in error["error"]
    assert "figma login" in error["error"]


def test_jira_schema_needs_no_credentials(goat_root: Path, capsys, monkeypatch):
    monkeypatch.chdir(goat_root)
    monkeypatch.delenv("JIRA_API_TOKEN", raising=False)
    assert main(["--root", str(goat_root), "jira", "schema"]) == 0
    payload = json.loads(capsys.readouterr().out)
...
def test_repos_command(goat_root: Path, capsys, monkeypatch):
    monkeypatch.chdir(goat_root)
    assert main(["--root", str(goat_root), "repos"]) == 0
    payload = json.loads(capsys.readouterr().out)
...
def test_catalog_and_workspace_generate(goat_root: Path, capsys, monkeypatch):
    ...
    assert main(["--root", str(goat_root), "workspace", "generate"]) == 0
...
def test_clone_dry_run_and_doctor(goat_root: Path, capsys, monkeypatch):
    monkeypatch.chdir(goat_root)
    assert (
        main(["--root", str(goat_root), "clone", "--dry-run", "--only", "frontend"])
        == 0
    )
    payload = json.loads(capsys.readouterr().out)
    ...
    assert doctor["sibling_root"] == str(goat_root.parent.resolve())
...
def test_jira_mine_uses_current_user_jql(goat_root: Path, capsys, monkeypatch):
    ...
    monkeypatch.setattr("goat.cli._client", lambda: FakeClient())
    assert main(["--root", str(goat_root), "jira", "mine"]) == 0
    payload = json.loads(capsys.readouterr().out)
...
def test_root_flag_works_before_or_after_subcommand(
    ...
    assert main(["repos", "--root", str(goat_root)]) == 0
    after = json.loads(capsys.readouterr().out)
    assert before["sibling_root"] == after["sibling_root"]
    assert before["sibling_root"] == str(goat_root.parent.resolve())


def test_error_is_json_on_stderr(goat_root: Path, capsys, monkeypatch):
    monkeypatch.chdir(goat_root)
    assert main(["--root", str(goat_root), "workspace", "path", "nope"]) == 1
...
def test_clone_placeholder_exits_nonzero(
    ...
    monkeypatch.chdir(goat_root)
    assert main(["--root", str(goat_root), "clone", "--only", "frontend"]) == 1
    error = json.loads(capsys.readouterr().err)
    assert error["repos"][0]["action"] == "blocked"


def test_missing_jira_env(goat_root: Path, capsys, monkeypatch):
    monkeypatch.chdir(goat_root)
    ...
    assert main(["--root", str(goat_root), "jira", "get", "WEB-1"]) == 1
    error = json.loads(capsys.readouterr().err)
