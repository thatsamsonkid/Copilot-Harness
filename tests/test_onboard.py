...
from goat.onboard import onboarding_steps, run_init
from goat.uv_check import detect_uv
...
def test_init_lists_missing_jira_keys_without_reading_secrets(
    goat_root: Path, catalog, monkeypatch, capsys
):
    _clear_jira_env(monkeypatch)
    example = goat_root / ".env.example"
    example.write_text(
        "JIRA_BASE_URL=\nJIRA_EMAIL=\nJIRA_API_TOKEN=\n",
        encoding="utf-8",
    )
    monkeypatch.chdir(goat_root)
    assert main(["--root", str(goat_root), "init"]) == 0
...
def test_interactive_init_stores_token_in_keychain(
    goat_root: Path, catalog, monkeypatch, isolated_keychain
):
    _clear_jira_env(monkeypatch)
    answers = iter(
        [
    ...
    env_text = (goat_root / ".env").read_text(encoding="utf-8")
    ...
    assert isolated_keychain.get_password("goat", "jira-api-token") == (
        "ATLASSIAN-SECRET"
    )
...
def test_interactive_init_falls_back_to_env_without_keychain(
    ...
    from goat.keychain import UnavailableStore, set_store
...
def test_init_treats_keychain_token_as_present(
    ...
    isolated_keychain.set_password("goat", "jira-api-token", "hidden")
...
def test_onboarding_uv_step_uses_os_specific_action(goat_root: Path, catalog):
    missing = detect_uv(which=lambda _name: None, system="Windows")
    steps = onboarding_steps(catalog, goat_root, uv=missing)
